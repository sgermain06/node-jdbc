...
class Statement {
    ...
    executeUpdate(sql, arg1) {
        return promiseForMethod(() => {
            if (!(isString(sql) && isNil(arg1))) throw new Error('INVALID ARGUMENTS');
            return this.s.executeUpdateSync.apply(this.s, [sql, arg1]);
        });
    }
    executeQuery(sql) {
        return promiseForMethod(() => {
            if (!isString(sql)) throw new Error('INVALID ARGUMENTS');
            return new ResultSet(this.s.executeQuerySync(sql));
        });
    }
    execute(sql) {
        return promiseForMethod(() => {
            if (!isString(sql)) throw new Error('INVALID ARGUMENTS');
            const isResultSet = this.s.executeSync(sql);
            if (isResultSet) {
